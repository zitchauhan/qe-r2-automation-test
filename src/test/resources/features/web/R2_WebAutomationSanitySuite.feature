...
@R2_Web @R2_Regression @R2_All @P-High @C-MyAccount @KER-4011 @ZYP_MYACCOUNT_K4011-10301 @CR-SK @AutomationSanityR2
...
@R2_Web @R2_Regression @R2_All @P-Highest @C-MyAccount @KER-4249 @ZYP_MYACCOUNT_K4249-10149 @CR-RK @AutomationSanityR2
...
@R2_Web @R2_Regression @R2_All @P-High @C-My_Account @KER-3093 @ZYP_CART_K3093-10418 @CR-SK @AutomationSanityR2
Scenario: TC_4-Verify that user is able to add a new gift card in My Account 
	Given user launches the browser and navigates to "ASO_HOME" page 
	And user clicks on SignIn link from global header 
	And user logs in as "RawUser" 
...
Scenario: TC_5-Verify promotion got applied for order level when user applied %off promotion on order -- Implicit promotion
	...
	|# verify Promo code discount is applied|	
		|Promocode_Txt|	
...
@R2_Web @R2_Regression @R2_All @P-Highest @C-Cart @KER-2872 @ZYP_CART_K2872-8710 @CR-DPK @AutomationSanityR2
	Scenario: TC_15-Verify that the user should be able to search the store details by entering ZIP code 
		Given user launches the browser and navigates to "ASO_HOME" page 
